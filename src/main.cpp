...
void savePlot(const std::string& filename,
    ...
    for (int x = ML; x <= ML+pw; x++) drawLine(img, x, MT+ph, x, MT+ph, {200,200,200});
    for (int y = MT; y <= MT+ph; y++) drawLine(img, ML, y, ML, y, {200,200,200});

    drawLine(img, ML, MT, ML, MT+ph, {0,0,0});
    drawLine(img, ML, MT+ph, ML+pw, MT+ph, {0,0,0});

    auto toY = [&](double v, double vmin, double vmax) {
        return MT + ph - (int)((v - vmin) / (vmax - vmin) * ph);
    };
    ...
    if (logScale) {
        vmin = 0;
        vmax = 0;
        for (auto v : t1) if (v > 0) vmax = std::max(vmax, std::log10(v));
        for (auto v : t2) if (v > 0) vmax = std::max(vmax, std::log10(v));
        for (auto v : t3) if (v > 0) vmax = std::max(vmax, std::log10(v));
        vmin = vmax;
        for (auto v : t1) if (v > 0) vmin = std::min(vmin, std::log10(v));
        for (auto v : t2) if (v > 0) vmin = std::min(vmin, std::log10(v));
        for (auto v : t3) if (v > 0) vmin = std::min(vmin, std::log10(v));
    } else {
        vmin = 0;
        vmax = *std::max_element(t1.begin(), t1.end());
    }

    auto getV = [&](double v) { return logScale && v > 0 ? std::log10(v) : v; };
    ...
    for (int i = 1; i < n; i++) {
        int x0 = ML + (int)((ns[i-1]-xmin)/(xmax-xmin)*pw);
        int x1 = ML + (int)((ns[i]  -xmin)/(xmax-xmin)*pw);
        drawLine(img, x0, toY(getV(t1[i-1]),vmin,vmax), x1, toY(getV(t1[i]),vmin,vmax), c1);
        drawLine(img, x0, toY(getV(t2[i-1]),vmin,vmax), x1, toY(getV(t2[i]),vmin,vmax), c2);
        drawLine(img, x0, toY(getV(t3[i-1]),vmin,vmax), x1, toY(getV(t3[i]),vmin,vmax), c3);
    }

    for (int i = 0; i < n; i += 4) {
        int x = ML + (int)((ns[i]-xmin)/(xmax-xmin)*pw);
